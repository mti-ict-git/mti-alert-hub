...
- wellness detail monitoring now exposes per-device `Next Run` plus schedule state in `Recipients`, while `Wellness Activity` shows the same timing in a more technical policy table for troubleshooting staggered, snoozed, or waiting-first-run behavior

...
- Support hybrid roles:
  ...
  - `Local Operator`
  - `Management Viewer`
- Enforce scope restrictions by site, department, section, or assigned area.
...
- Support hybrid organization sourcing through import, synchronization, and controlled manual override.
...
2. User selects content type, priority, audience, channels, and response workflow.
3. System resolves and previews audience.
4. User publishes now or schedules.
5. System creates delivery jobs.
6. Delivery jobs are executed per channel.
7. Status becomes visible in monitoring screens.
...
5. Management views response progress in real time.
6. In MVP, non-response escalation remains recipient-only, such as re-alerting or re-attempting the same recipient.

### Workflow 4: Recipient Response
1. Recipient receives the communication.
2. Recipient opens or views the message.
3. Recipient submits a workflow-defined response.
4. System stores the response and updates monitoring status.

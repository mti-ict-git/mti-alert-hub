...
- Canonical file naming does not mean reusing the same semantic version for different payloads. Any installable MSI payload change should be published with a new `PackageVersion`, while `ProductCode` or `PackageCode` are treated as diagnostics rather than operator-facing rollout identity.
...
- The backend container must mount durable storage at `/app/backend/local-packages`; the current standalone deployment baseline falls back to the named Docker volume `mti-alert-backend-local-packages`, but when a single Compose-compatible `backend_local_packages` volume still exists from the older deployment path the script now auto-prefers that legacy volume so uploaded rollout packages continue using the established storage without needing `--packages-volume`.
...
## Policy application route startup fix (2026-09-17)
The new `POST /agent/reminder-policies/{policyId}/application` device endpoint was missing from the exact non-administrative route manifest. Managed-access startup therefore stopped with `Administrative route has no access policy`. The endpoint is now explicitly classified alongside reminder policy events; there is no wildcard exemption. The controller still requires a bearer token, and AgentService still validates the device session, current authorization, policy ownership and version.
The backend Docker builder now executes the complete route inventory/access-policy regression suite before compiling, so future manifest drift fails image build before deployment replaces a running container. Verification: the original route inventory failure was reproduced; all nine access-route and policy-application tests passed after the fix; backend compilation passed; compiled agent routes passed the same startup classification function without database access. Missing/malformed bearer tokens and invalid-session propagation are covered. No migration or agent rebuild is needed for this fix. Server image rebuild/redeploy remains pending; it was not performed from this workspace.


## Standalone deployment package storage and relay correction (2026-09-17)
The previous standalone deploy script omitted package-relay and used a new hardcoded named volume instead of the Compose package volume. Operator-provided Docker inspection confirmed the deployed backend used `mti-alert-backend-local-packages`, while the still-running Compose relay used `mti-alert-hub_backend_local_packages`. The old volume still exists; its file contents have not yet been inspected. The normal deploy path does not delete package volumes. `destroy --with-postgres` remains a separate destructive PostgreSQL-volume operation.
The corrected script discovers existing Compose/current backend package volumes before replacement, reuses a single unambiguous volume, and now automatically prefers a single Compose-compatible `backend_local_packages` volume when both that legacy storage and the newer standalone default volume are present. Only genuinely ambiguous multi-volume cases still require `--packages-volume NAME`. Existing bind mounts require explicit storage migration rather than silent replacement. No automatic copying, merging or deletion is performed. When GitHub configuration is present, relay secrets/repository/thumbprint are validated before container replacement, the relay image is built and a relay is started with the same selected volume as the backend. Only a prior Compose relay using that selected volume is replaced, avoiding two writers competing for its lock. Status/logs/stop include the standalone relay. Its healthcheck verifies a fresh worker heartbeat; this does not prove a GitHub import succeeded.
For the inspected server, after synchronizing the corrected source, run:

```bash
bash scripts/deploy-docker.sh --skip-migrations --packages-volume mti-alert-hub_backend_local_packages
```
Inspect `/packages` in the old relay before recovery if package contents need confirmation. Keep both volumes; do not prune or merge them. Any packages newly uploaded into the standalone volume remain there and require separate reviewed reconciliation if needed.
The script also refuses to remove unrelated Docker containers merely because they own a required host port. Verification: Bash syntax passed, parsed-environment launch regression passed, and fake-Docker storage/relay tests passed (legacy volume discovery, ambiguity refusal, explicit selection, fresh install, shared-volume/secrets mounts, and unrelated-container protection). No real Docker deployment or server volume mutation was performed from this workspace. Production recovery and package-list/relay-import acceptance remain pending.

...
- now using the approved OHIH outcome contract: `GotIt` and `Done` emit `Completed`, `RemindMeLater` emits `Snoozed` with terminal outcome `Deferred`, `Start` and `Next` remain progress evidence, and both the window close button and a `Close` CTA emit `Dismissed`
- terminal wellness CTA events include normalized `terminalOutcome` metadata, while the server retains compatibility handling for historical `Completed + Close` evidence from older packages

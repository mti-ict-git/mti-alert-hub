...
const originalWorkflowTimeoutRows = await db.query(
  `select escalation_timeout_minutes from public.response_workflows where id = $1::uuid`,
  [workflowId],
);
const originalWorkflowTimeout = originalWorkflowTimeoutRows.rows[0]?.escalation_timeout_minutes ?? null;
...
try {
  await db.query(
    `update public.response_workflows set escalation_timeout_minutes = 0 where id = $1::uuid`,
    [workflowId],
  );
  ...
  const device =
    devices.items.find((item) => item.deviceIdentifier === "device-mti-ops-01") ?? devices.items[0];
  if (!device) {
  ...
  await db.query(
    `update public.response_workflows set escalation_timeout_minutes = $2 where id = $1::uuid`,
    [workflowId, originalWorkflowTimeout],
  );
  await db.end();
}

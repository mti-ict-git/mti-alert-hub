  ...
  return (
    <div className="rounded-lg border bg-[#0b141a] p-3 text-[13px] text-[#e9edef]">
      ...
      <div className={cn("max-w-[320px] rounded-lg rounded-tl-none bg-[#202c33] p-3 shadow")}>
        <div className="font-semibold text-[#00a884]">🚨 MTI CONNECT</div>
        <div className="mt-1">
          <div>
            <span className="text-[#8696a0]">Priority:</span> {priority}
          </div>
          {site && (
            <div>
              <span className="text-[#8696a0]">Location:</span> {site}
            </div>
          )}
          <div>
            <span className="text-[#8696a0]">Title:</span> {title || "—"}
          </div>
          {instruction && (
        ...
        <div className="mt-2 text-right text-[10px] text-[#8696a0]">
